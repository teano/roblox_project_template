...
- При включённых слотах `AdmissionRouting` резервирует конкретный `SlotId`
  атомарно в `MemoryStoreService` и для местного владельца, и для удалённого
  источника. Координатор проверяет подтверждение перед общей фиксацией.
  При выключенных слотах запросы хранилища и собственный подбор не запускаются.
- Сервер сначала проверяет местную вместимость после инициализации. Первый
  вход не требует другого активного сервера. При нехватке используются точный
  резерв, ограниченный подбор и согласованное создание резервного сервера.
- Прибытие расходует точный пользовательский резерв текущего поколения и
  заменяет его жетон. Поздний отказ источника не освобождает занятое место.
  Продление не стирает чужие резервы; восстановление записи сохраняет местных
  владельцев и меняет поколение. Все внешние ожидания ограничены.
- Для этой границы читать [межсерверный допуск](../../docs/AdmissionRouting.md)
  и [ADR-0053](../../docs/adr/template/0053-atomic-cross-server-slot-admission.md).
