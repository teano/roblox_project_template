...
2. Resolve the requested feature by stable ID, slug, folder, or exact title. Never infer a different feature from the branch name.
...
4. Run from the repository root:
   ...
   For a new feature, pass `-Title` and an ASCII `-Slug` when its requested name cannot produce one. For ready work, pass `-ReopenReason` only after explicit user authorization.
...
7. Confirm the assigned ID, branch, base commit, task link, blockers, and next allowed step before editing source.
