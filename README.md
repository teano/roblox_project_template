...
- `$feature-start <название или ID>` — начать новую запланированную фичу или
  явно переоткрыть готовую;
- `$feature-continue <ID>` — продолжить приостановленную фичу и восстановить
  контекст из манифеста, handoff, Git, правил, ADR и связанных задач;
- `$feature-pause <ID>` — сохранить checkpoint, освободить writer lease, но
  оставить ветку зарезервированной фичей;
- `$feature-finish <ID>` — выполнить полный аудит и только после всех проверок
  перевести фичу в состояние `ready`.

В одной ветке может находиться только одна фича со статусом `in_progress`.
Git-хуки намеренно не используются: проверки запускает агент явно из
