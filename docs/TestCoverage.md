...
Межсерверный допуск проверяет `AdmissionRoutingTestRunner` вместе с наборами
координатора, слотов и телепортации. Общее подменное хранилище воспроизводит
повтор атомарной операции при конфликте: два источника на последний слот,
конкурирующий местный вход, совместное создание резервного сервера,
первый запуск, отключённая стратегия, истечение, восстановление поколения,
поздняя отмена и остановка. Серверный код доступа не попадает в конверт.
Настоящий переход и приглашение в резервный сервер требуют отдельно
опубликованных клиентов Roblox; см. [межсерверный допуск](AdmissionRouting.md).
